...
public final class BuscaElem {
	...
	public static <T extends Comparable<? super T>> T kesimoRec(T v[], int izq, int der, int k) {
		...
		if (izq < der) {
			int partitionIndex = OrdenacionRapida.partir(v, v[der], izq, der);
			// System.out.println(partitionIndex + " " + k + " " + v[k]);
			if (k == partitionIndex)
				return v[k];
			else if (k < partitionIndex)
				return kesimoRec(v, izq, partitionIndex - 1, k);
			else
				return kesimoRec(v, partitionIndex + 1, der, k);
		}
		return v[k];

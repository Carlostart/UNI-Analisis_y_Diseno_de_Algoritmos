...
public class ProblemaMochila {
	...
	/**
	 * Construye un problema de la machila
	 * @param pesos
	 * @param valores
	 * @param pesoMaximo
	 */
	private void init(int[] pesos, int[] valores, int pesoMaximo) {
		if (pesos!=null && valores!=null && pesos.length == valores.length && pesoMaximo >= 0) {
			this.pesoMaximo = pesoMaximo;
			items = new ArrayList<Item>(pesos.length);
			for (int i=0; i<pesos.length; i++) {
				Item item = new Item(i, pesos[i], valores[i]);
				items.add(item);
			}
		}
	}
	 ...
	 * @param pesoMaximo
	 */
	public ProblemaMochila(int[] pesos, int[] valores, int pesoMaximo) {
		init(pesos, valores, pesoMaximo);
	}
